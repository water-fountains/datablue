...
function p<WC, OC, WER, OER, W extends SourceConfig<WC, WER> | null, O extends SourceConfig<OC, OER> | null>(
  p: FountainPropertyMetaUnfinished<W, O>
): FountainPropertyMeta<WC, OC, WER, OER, W, O> {
  return {
...
const fountain_properties: FountainPropertiesMeta = {
  name: p({
    // unique codename of property
    name: {
      // name of property in all supported languages
      en: 'title',
      de: 'Titel',
      fr: 'titre',
      it: 'titolo',
    ...
    descriptions: {
      // description of what property represents
        ...
        value_translation: values => {
          ...
          if (null != values) {
            ...
            for (let i = 0; i < values.length; i++) {
              const c = values[i].value; //we don't need the qualifiers here
              if (!catSet.has(c) && !isBlacklisted(c)) {
                catSet.add(c);
                const cat = { s: 'wd', c: c, l: -1 };
                if (0 < i) {
...
_.forEach(fountain_properties, (property, key) => {
  property.id = key;
});
