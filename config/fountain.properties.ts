...
interface FountainPropertyMeta<
  WC = any[],
  OC = string, 
  WER= string, 
  OER= string, 
...
type FountainPropertyMetaWikiOnly< WC = any[], WER= string> = FountainPropertyMeta<WC, string, WER, string,  SourceConfig<WC, WER>, null>
type FountainPropertyMetaOsmOnly< OC = any[], OER= string> = FountainPropertyMeta<any[], OC, string,  OER, null, SourceConfig<OC, OER>>

export type FountainPropertiesMeta = {
...
function p<WC, OC, WER, OER, W extends SourceConfig<WC, WER> | null, O extends SourceConfig<OC, OER> | null>(p: FountainPropertyMetaUnfinished<W, O>): FountainPropertyMeta<WC, OC, WER, OER, W, O>{
  return {
    id: '', // will be set afterwards
    value: null,  // the default value is "null"
    comments: '',
    status: PROP_STATUS_WARNING,  // the property status is a warning if no data is available. This is updated when the data is fetched
    source: '',  // this indicates the source from which the property value was obtained.
    ...p,
  }
}

const fountain_properties: FountainPropertiesMeta = {
  name: p({  // unique codename of property
  ...
  id_operator: p({
    ...
    src_config: {
      wikidata: {
        ...
        src_info: {
          en:`The catalog code must have a 'catalog' qualifier referring to the catalog documented in the location metadata. (${_.map(locations, (loc,_name)=>{ const l = loc!!; return `${l.name}: ${l.operator_fountain_catalog_qid}`}).join(', ')})`,
          de: `Der Katalogcode muss einen 'Katalog'-Qualifizierer haben, der sich auf den in den Standortmetadaten dokumentierten Katalog bezieht. (${_.map(locations, (loc,_name)=>{const l = loc!!; return `${l.name}: ${l.operator_fountain_catalog_qid}`}).join(', ')})`,
          fr: `Le code de catalogue doit avoir un qualificatif \'catalogue\' faisant référence au catalogue documenté dans les métadonnées de localisation. (${_.map(locations, (loc,_name)=> {const l = loc!!; return `${l.name} : ${l.operator_fountain_catalog_qid}`}).join(', ')})`,
          it: `Il codice del catalogo deve avere un qualificatore \'catalogo\' che faccia riferimento al catalogo documentato nei metadati della località. (${_.map(locations, (loc,_name)=> {const l = loc!!; return `${l.name} : ${l.operator_fountain_catalog_qid}`}).join(', ')})`,
          tr: `Katalog kodunun kiralık üstveride kataloğa kaydetmek için kataloğu olmalı. (${_.map (locations, (loc, _name) => {const l = loc!!; return `${l.name}: ${l.operator_fountain_catalog_qid}`}). join (', ')}) `
          },
      ...
      wikidata: {
        ...
        value_translation: values => {
          //TODO introduce own type
        	let cats : {s: string, c: any, l: number}[] = [];
        	if (null != values) {
        	  let catSet = new Set();
...
// override id with the property name
_.forEach(fountain_properties, (property, key) =>{  property.id = key;  })
 ...
 */
export function get_prop(fountain, source, property: string) {
