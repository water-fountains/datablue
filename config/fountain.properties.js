  ...
  id_operator: {
    ...
    src_config: {
      wikidata: {
        ...
        src_info: {
          en:`The catalog code must have a 'catalog' qualifier referring to the catalog documented in the location metadata. (${_.map(locations, (l,name)=>{return `${name}: ${l.operator_qid}`}).join(', ')})`,
          de: `Der Katalogcode muss einen 'Katalog'-Qualifizierer haben, der sich auf den in den Standortmetadaten dokumentierten Katalog bezieht. (${_.map(locations, (l,name)=>{return `${name}: ${l.operator_qid}`}).join(', ')})`,
          fr: `Le code de catalogue doit avoir un qualificatif \'catalogue\' faisant référence au catalogue documenté dans les métadonnées de localisation. (${_.map(locations, (l,name)=> {return `${name} : ${l.operator_qid}`}).join(', ')})`
          },
