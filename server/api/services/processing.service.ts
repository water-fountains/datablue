...
import {PROP_STATUS_INFO, LAZY_ARTIST_NAME_LOADING_i41db//, LANGS
...
export function essenceOf(fountainCollection: FountainCollection) {
  ...
  let essentialPropNames: string[] = _.map(fountain_property_metadata, (p, p_name)=>{if (p.essential) {return p_name} else return null}).filter((p): p is string => p !== null);
  let withGallery = 0;
  let strVw = 0;
...
export function fillInMissingWikidataFountains(osm_fountains: FountainConfig[] , wikidata_fountains: FountainConfig[], dbg: string): Promise<FountainConfigCollection>{
